...
async function boot(): Promise<{ ctx: Context; settingsFiber: Fiber; pluginFiber: Fiber }> {
  ...
  const pluginFiber = ctx.plugin(searxngPlugin, { baseURL: 'http://search.entry.test:10000' })
...
describe('web-search-searxng settings section', () => {
  it('serves a stored endpoint to the next search without re-registering the provider', async () => {
    ...
    expect(await searchOnce(bench.ctx)).toContain('http://search.entry.test:10000')

    await bench.ctx.settings.update(WEB_SEARCH_SEARXNG_SETTINGS_NAMESPACE, {
      baseURL: 'http://search.stored.test:10001',
    })

    expect(await searchOnce(bench.ctx)).toContain('http://search.stored.test:10001')
    await bench.ctx.fiber.dispose()
  })

  it('keeps the literal token out of every described layer', async () => {
    const bench = await boot()
    await bench.ctx.settings.update(WEB_SEARCH_SEARXNG_SETTINGS_NAMESPACE, { apiToken: 'sx-stored-secret' })

    const [descriptor] = bench.ctx.settings.describe({ redactSecrets: true })
      .filter(row => String(row.ns) === 'web-search-searxng')

    expect(JSON.stringify(descriptor)).not.toContain('sx-stored-secret')
    expect(descriptor?.secrets).toEqual([{ path: ['apiToken'], set: true }])
    await bench.ctx.fiber.dispose()
  })
